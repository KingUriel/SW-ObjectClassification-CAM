...
async function plotSalience(image, model) {
    ...
    const gradients = await layerCam(x, model, classIndex);
    drawSaliencyMap(gradients);
    const endCAM = performance.now();
    console.info(`Layer-CAM took ${endCAM - startCAM} ms`, tf.memory());
}
...
async function layerCam(images, model, classIndex) {
    ...
    // Create two models: one for the CNN part and one for the task-specific part
    ...
    // Define function to get class output
    ...
    // 1. Keep only positive gradients (ReLU)
    const positiveGrads = grads.relu();

    // 2. Calculate weights for each spatial location
    // We use the positive gradients as weights directly
    const weights = positiveGrads;

    // 3. Weighted sum of feature maps
    const weightedFeatureMaps = featureMaps.mul(weights);

    // 4. Sum across channels and apply ReLU
    const cam = weightedFeatureMaps.sum(-1).relu();
